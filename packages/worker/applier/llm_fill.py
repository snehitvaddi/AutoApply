...
def build_fill_prompt(
    *,
    ...
    """Construct the prompt asking Claude to produce a fill plan."""
    ak_json = json.dumps(answer_key, indent=2)[:3500]
    jd_block = f"\nJOB DESCRIPTION (for tone / company-specific answers):\n{jd_hint[:2000]}\n" if jd_hint else ""

    return f"""You are filling out a job application form for this user.
Reply with STRICT JSON ONLY — no prose, no markdown, no explanation.
If you cannot see any fields to fill, return {{"fills": []}}.
...
{ak_json}
{jd_block}
FORM SNAPSHOT (accessibility tree; every field has [ref=eXX]):
{snapshot_text[:9000]}
...
- Submit_ref: the ref of the final submit (NOT "Next" unless this is the
  only/last page).
- If the form has a "Next" button and the page you see is clearly step 1
  of N, set submit_ref to the Next button's ref — the driver will
  re-snapshot after and call you again for step 2.
"""
    ...
    *,
    ...
    try:
        ...
        for step in range(max_steps):
            ...
            if plan is None:
                # Claude unreachable mid-flow — caller should NOT retry via
                # legacy (we've already navigated + maybe partial-filled).
                # Best we can do is fail this one job and let worker retry.
            ...
            logger.info(
                ...
                f"{len(plan.get('fills', []) or [])} fills, "
                f"{len(plan.get('dropdowns', []) or [])} dropdowns, "
            ...
            if not submit_ref:
                # Claude didn't identify a submit/next — treat as inability
                return {
                    "success": False, "screenshot": take_screenshot(),
            ...
            # No confirmation yet — may be multi-step. Loop.
